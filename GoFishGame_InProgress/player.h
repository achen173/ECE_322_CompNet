 ...
 */
struct player {
  struct hand* card_list; //struct linked_list* list = NULL;
  char book[7];
  size_t hand_size;
};
...
struct card transfer_card(int ID, char found_card_rank);
/*
 * Function: add_card
 * -------------------
...
struct hand* add_OneCard_To_Hand(struct hand* ThePointer, struct card TheCard);
char ask_for_input(int user, struct player your_hand, struct player opponent_hand);
// int add_to_list(struct hand* target, struct card new_item);
/*
 ...
 *  returns: return 0 if no error, non-zero otherwise
 */
struct hand* remove_from_list(int k, struct hand* target, char item);
struct hand* remove_card(int ident, struct hand* target, char rank);
 ...
 */
int search(struct hand* target2, char rank[]);
int newsearch25(struct hand* target3, char rank);
/*
