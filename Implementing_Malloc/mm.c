// Alan Chen
// 11/16/2019
// I have attempted the extra credit. I am unsure if I am correct since the test case keeps giving me errors.
...
void* mm_malloc (size_t size) {
  ...
  // Search free list for block of requested size (1)
  ptrFreeBlock = (BlockInfo*) searchFreeList(reqSize);
  // If no block, then need to request more space, and re-search (2)
  if(ptrFreeBlock == NULL){
    requestMoreSpace(reqSize);
  }
  ptrFreeBlock = (BlockInfo*) searchFreeList(reqSize);
  // Check the size of the block (3)
  blockSize = SIZE(ptrFreeBlock->sizeAndTags);
  size_t extra_space = blockSize - reqSize;
  // Was the preceeding block used? Need to know for below (4)
  size_t precedingBlockUsed = ptrFreeBlock->sizeAndTags & TAG_PRECEDING_USED;
  //Check to see if block has extra space, if so we need to split (5A)
  if(extra_space >= MIN_BLOCK_SIZE){
    // Split the free block, remove the allocated block from list (6A)
    BlockInfo* nextBlock = (BlockInfo*) UNSCALED_POINTER_ADD(ptrFreeBlock, reqSize);
    removeFreeBlock(ptrFreeBlock);
    // Update headers/footers of both new blocks (7A)
    ptrFreeBlock->sizeAndTags = reqSize;  // update block 1 size
    ptrFreeBlock->sizeAndTags = ptrFreeBlock->sizeAndTags | TAG_USED; // update block 1 Tag_use
    ptrFreeBlock->sizeAndTags = ptrFreeBlock->sizeAndTags | precedingBlockUsed;
    *((size_t*) UNSCALED_POINTER_ADD(ptrFreeBlock, reqSize-WORD_SIZE)) = ptrFreeBlock->sizeAndTags; // update block 1 footer
    nextBlock->sizeAndTags = extra_space;  // update block 2 size
    nextBlock->sizeAndTags = nextBlock->sizeAndTags & ~TAG_USED; // update block 2 Tag_use
    nextBlock->sizeAndTags = nextBlock->sizeAndTags | TAG_PRECEDING_USED; // update block 2 Tag_Preceding_used
    *((size_t*) UNSCALED_POINTER_ADD(ptrFreeBlock, blockSize-WORD_SIZE)) = nextBlock->sizeAndTags; // update block 2 footer
    // Insert new split free block into free block list (8A)
    insertFreeBlock(nextBlock);
    // If no extra space, no need to split (5B)
  }else{
    // Update block header to be used (6B)
    ptrFreeBlock->sizeAndTags = ptrFreeBlock->sizeAndTags | TAG_USED;
    // Update adjacent block header (7B)
    ...
    *((size_t*) UNSCALED_POINTER_ADD(ptrFreeBlock, blockSize-WORD_SIZE)) = ptrFreeBlock->sizeAndTags;
    // Remove block from free list (8B)
    removeFreeBlock(ptrFreeBlock);
  }
  ...
  // Return pointer (9)
  return (BlockInfo*)UNSCALED_POINTER_ADD(ptrFreeBlock, WORD_SIZE);
}

/* Free the block referenced by ptr. */
void mm_free (void *ptr) {  // the pointer passed in is to the payload
  size_t payloadSize;
  BlockInfo * blockInfo;
  ...
  // Find the header for the free’d block (1)
  blockInfo = (BlockInfo*) UNSCALED_POINTER_SUB(ptr, WORD_SIZE);
  // Determine the size of the payload (2)
  payloadSize = SIZE(blockInfo->sizeAndTags);
  // Find the location of the boundary tag that you need to add (3)
  size_t current_footer = *((size_t*) UNSCALED_POINTER_ADD(blockInfo, payloadSize-WORD_SIZE));
  // Find the next block’s header (4)
  followingBlock = (BlockInfo*) UNSCALED_POINTER_ADD(blockInfo, payloadSize);
  // Mark the block as unallocated in header and footer (5)
  blockInfo->sizeAndTags = blockInfo->sizeAndTags & ~TAG_USED;  // for header
  *((size_t*) UNSCALED_POINTER_ADD(blockInfo, payloadSize-WORD_SIZE)) = blockInfo->sizeAndTags;  // for footer
  // Mark the preceding block as not used in header of the next block (6)
  followingBlock->sizeAndTags = followingBlock->sizeAndTags & ~TAG_PRECEDING_USED;
  // Insert the free block into our list (7)
  insertFreeBlock(blockInfo);
  // Coalesce the free block (8)
...
void* mm_realloc(void* ptr, size_t size) {
  /*
  * Purpose: The realloc() function changes the size of the memory block pointed to by
  ...
  * 
  * Condition 1: If ptr is NULL, then the call is equivalent to malloc(size), for all values of size 
  * Condition 2: If size is equal to zero, and ptr is not NULL, then the call is equivalent to free(ptr).
  * Condition 3: If the new size == old size, then just return pointer
  * Condition 4: If the new size is smaller than the old size, remove the extra space into the free list
  * Condition 5: If the new size is larger than the old size, the added memory will not be initialized. 
      * If the area pointed to was moved, a free(ptr) is done.
  */
  if(ptr == NULL){  // Condition 1
    printf("%s \n", "YOUR POINTER IS NULL");
    return mm_malloc(size);
  }
  if(ptr != NULL && size == 0){ // Condition 2
    printf("%s \n", "YOUR SIZE IS 0");
    mm_free(ptr);
    return NULL;
  } 
  BlockInfo* blockInfo = (BlockInfo*) UNSCALED_POINTER_SUB(ptr, WORD_SIZE);
  size_t precedingBlockUsed = blockInfo->sizeAndTags & TAG_PRECEDING_USED;
  size_t blockSize = SIZE(blockInfo->sizeAndTags);
  if(blockSize ==  size){  // Condition 3
    printf("%s \n", "YOUR SIZE IS EXACTLY THE ORIGINAL SIZE");
    return ptr;
  }
  if (blockSize > size) {  // Condition 4
    printf("%s \n", "YOUR SIZE IS LARGER THAN THE GIVEN SIZE (START SHRINKING)");
    size_t reqSize;
    size += WORD_SIZE;
    if (size <= MIN_BLOCK_SIZE) {
      reqSize = MIN_BLOCK_SIZE;
    ...
    size_t extra_space = blockSize - reqSize;
    if(extra_space >= MIN_BLOCK_SIZE){ 
      // Update info for current block
      blockInfo->sizeAndTags = reqSize;  // update block 1 size
      blockInfo->sizeAndTags = blockInfo->sizeAndTags | TAG_USED;  // update header to be used
      blockInfo->sizeAndTags = blockInfo->sizeAndTags | precedingBlockUsed;
      *((size_t*) UNSCALED_POINTER_ADD(blockInfo, reqSize-WORD_SIZE)) = blockInfo->sizeAndTags; // update footer
      // Update info for the next new block
      BlockInfo* next_blockInfo = (BlockInfo*) UNSCALED_POINTER_ADD(blockInfo, reqSize);
      next_blockInfo->sizeAndTags = extra_space;
      next_blockInfo->sizeAndTags = next_blockInfo->sizeAndTags & ~TAG_USED;
      next_blockInfo->sizeAndTags = next_blockInfo->sizeAndTags | TAG_PRECEDING_USED;
      *((size_t*) UNSCALED_POINTER_ADD(next_blockInfo, extra_space-WORD_SIZE)) = next_blockInfo->sizeAndTags; // update footer
      insertFreeBlock(next_blockInfo);
      return (BlockInfo*)UNSCALED_POINTER_ADD(blockInfo, WORD_SIZE);
    }
  }else{  // Condition 5
      BlockInfo* next_blockInfo = (BlockInfo*) UNSCALED_POINTER_ADD(blockInfo, blockSize);
      if(!(next_blockInfo->sizeAndTags & TAG_USED)){
        if(size > SIZE(next_blockInfo->sizeAndTags)+blockSize ){
          printf("%s \n", "NEED TO CONTINUE ADDING BLOCKS");
        }else{
          blockInfo->sizeAndTags = size;  // update block 1 size
          blockInfo->sizeAndTags = blockInfo->sizeAndTags | TAG_USED;
          blockInfo->sizeAndTags = blockInfo->sizeAndTags | precedingBlockUsed;
          *((size_t*) UNSCALED_POINTER_ADD(blockInfo, size-WORD_SIZE)) = blockInfo->sizeAndTags; // update footer
          size_t extra_space = (blockSize+SIZE(next_blockInfo->sizeAndTags))-size;

          BlockInfo* next_blockInfo = (BlockInfo*) UNSCALED_POINTER_ADD(blockInfo, size);
          next_blockInfo->sizeAndTags = extra_space;
          next_blockInfo->sizeAndTags = next_blockInfo->sizeAndTags & ~TAG_USED;
          next_blockInfo->sizeAndTags = next_blockInfo->sizeAndTags | TAG_PRECEDING_USED;
          *((size_t*) UNSCALED_POINTER_ADD(next_blockInfo, extra_space-WORD_SIZE)) = next_blockInfo->sizeAndTags; // update footer
